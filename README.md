...
| **03. Refine** | **`app_frame.py`**<br>*(Interactive Frame Station)* | • Load images and JSONs to visualize anomalies with red tick markers on slider<br>• Inspect identity consistency via **Coat Pattern Viewer** ($\pm 3$ frames)<br>• Fine-tune bounding box coordinates, angles, and cow IDs<br>• Changes are automatically saved back to corresponding JSON files | Required |
